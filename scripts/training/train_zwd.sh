#!/bin/bash
#SBATCH --job-name=zwd_1          # short name for the job
...
#SBATCH --output=train_esfm_lr1e-4_lw_1.out  # output log file
...
srun --ntasks=$nnodes \
     ...
     -u -l torchrun \
      ...
      train_fsdp.py --batch_size 1 \
        ...
        --log_dir "/path/to/checkpoints/zwd/lr_${LEARNING_RATE}_lw_1/" \
        --wnb_project "ESFM_climate" \
        --wnb_name "ESFM_zwd/$SLURM_JOB_NUM_NODES/lr_${LEARNING_RATE}_lw_1" \
        --wnb_id "$CURRENT_TIME" \
        --dataset_config_path "dataset_config.yaml" \
        --data_sources "era5_zwd" \
